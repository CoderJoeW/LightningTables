...
class TotalCostQueryTest : DockerComposeTestBase() {
    private val parser = LightningTableTriggerGeneratorSqlParser()
    private val query = queries["totalCost"]!!
    ...
    private fun queryOriginalTable(): BigDecimal {
        connect().use { conn ->
            val rs = conn.createStatement().executeQuery(query)
            return if (rs.next()) {
                rs.getBigDecimal("total_cost") ?: BigDecimal.ZERO
            } else {
                BigDecimal.ZERO
            }
        }
    ...
    private fun queryLightningTable(): BigDecimal {
        connect().use { conn ->
    ...
    fun `tables match after deleting the only transaction`() {
        ...
        assertTablesMatch("after deleting only transaction (sum should be 0)")
    }
    ...
    fun `tables match after deleting all transactions`() {
        ...
        assertTablesMatch("after deleting everything (sum should be 0)")
    }
    ...
    // --- Update scenarios (updates DO affect SUM) ---
    ...
    fun `tables match after changing user_id on a transaction`() {
        ...
        assertTablesMatch("after moving a transaction from user 1 to user 2 (sum unchanged)")
    }
    ...
    fun `tables match after updating both cost and user_id`() {
        ...
        assertTablesMatch("after updating both cost and user_id")
    }
    ...
    fun `tables match after full cycle of insert, update, delete`() {
        ...
        insertTransaction(2, 20.00)
        insertTransaction(3, 30.00)
        assertTablesMatch("after initial inserts (sum should be 60)")
        ...
        assertTablesMatch("after update on user 1 (sum should be 65)")
        ...
        assertTablesMatch("after deleting user 3's transactions")
    ...
    fun `tables match after rapid insert-update-delete sequence`() {
        ...
        assertTablesMatch("after delete (sum should be 0)")
    ...
    fun `tables match after bulk inserts then bulk deletes`() {
        ...
        assertTablesMatch("after deleting 25 of user 1's rows")
    }
    ...
    fun `tables match after inserting many then deleting all at once`() {
        ...
        assertTablesMatch("after deleting all (sum should be 0)")
    }
    ...
    fun `tables match with only zero-cost transactions`() {
        ...
        assertTablesMatch("with only zero-cost transactions (sum should be 0)")
    }
    ...
    fun `tables match after deleting zero-cost transactions`() {
        ...
        assertTablesMatch("after deleting zero-cost rows")
    }
    ...
    fun `tables match after many mixed operations`() {
        ...
        // Insert phase
        repeat(100) { insertTransaction(1, 1.50) }
        repeat(100) { insertTransaction(2, 2.50) }
        repeat(100) { insertTransaction(3, 3.50) }
        ...
        assertTablesMatch("after deleting 50 of user 1's rows")
        ...
        assertTablesMatch("after bulk update of user 2")

        // More inserts
        repeat(50) { insertTransaction(1, 0.50) }
        assertTablesMatch("after adding 50 more for user 1")

        // Delete all of user 3
        transaction {
            TransactionsTable.deleteWhere { TransactionsTable.userId eq 3 }
        }
        assertTablesMatch("after removing all user 3 transactions")
    }
}
