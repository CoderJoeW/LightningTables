...
class TotalRecordsByUserIdQueryTest : DockerComposeTestBase() {
    private val parser = LightningTableTriggerGeneratorSqlParser()
    private val query = queries["totalRecordsByUserId"]!!
    ...
    private fun queryOriginalTable(): Map<Int, Long> {
        connect().use { conn ->
    ...
    private fun queryLightningTable(): Map<Int, Long> {
        connect().use { conn ->
    ...
    @Test
    fun `tables match after many inserts for one user`() {
        setupTriggersAndLightningTable()
        repeat(100) {
            insertTransaction(1, 0.01)
        }
        assertTablesMatch("after 100 inserts for user 1")
    }
    ...
    @Test
    fun `tables match after deleting by cost filter across multiple users`() {
        setupTriggersAndLightningTable()
    ...
    // --- Update scenarios (updates don't affect count) ---
    ...
    // --- Update user_id (moves count between users) ---
    ...
    @Test
    fun `tables match after moving transaction to a user with no prior records`() {
        setupTriggersAndLightningTable()
        insertTransaction(1, 10.00)
        insertTransaction(1, 20.00)
    ...
    @Test
    fun `tables match after moving all transactions to a new user leaving source empty`() {
        setupTriggersAndLightningTable()
        insertTransaction(1, 10.00)
        insertTransaction(1, 20.00)
    ...
    @Test
    fun `tables match after consolidating all users into one`() {
        setupTriggersAndLightningTable()
        insertTransaction(1, 10.00)
        insertTransaction(2, 20.00)
    ...
    @Test
    fun `tables match after inserting and deleting repeatedly for same user`() {
        setupTriggersAndLightningTable()
    ...
    @Test
    fun `tables match after inserting many then deleting all for one user`() {
        setupTriggersAndLightningTable()
        repeat(50) { insertTransaction(1) }
        repeat(50) { insertTransaction(2) }
        ...
        assertTablesMatch("after deleting all of user 1 (user 1 should vanish)")
    }
    ...
    fun `tables match after update that does not change the value`() {
        ...
        assertTablesMatch("after no-op update (count unchanged)")
    }
    ...
    fun `tables match after many mixed operations`() {
        ...
        assertTablesMatch("after deleting 50 from user 1")
